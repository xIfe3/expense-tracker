      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap gap-3">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="h-12 w-36 animate-pulse rounded-xl bg-slate-100 sm:w-44" />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button onClick={openAdd} className="flex items-center justify-center gap-2 rounded-xl bg-slate-900 px-5 py-2.5 text-sm font-semibold text-white hover:bg-slate-800">
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2 sm:gap-3">
          {defaults.map((cat) => (
            <div key={cat.id} className="flex items-center gap-2.5 rounded-xl border border-slate-100 bg-white px-3 py-2.5 sm:px-4 sm:py-3">
      ...
      <div>
        ...
        {custom.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-slate-200 py-12 text-center">
            ...
            <button onClick={openAdd} className="mt-2 text-sm font-semibold text-blue-600 hover:text-blue-700">Create one</button>
          </div>
        ) : (
          <div className="flex flex-wrap gap-2 sm:gap-3">
            {custom.map((cat) => (
              <div key={cat.id} className="group flex items-center gap-2.5 rounded-xl border border-slate-100 bg-white px-3 py-2.5 sm:px-4 sm:py-3">
                <div className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: cat.color }} />
                <span className="text-xs font-medium text-slate-700 sm:text-sm">{cat.name}</span>
                <div className="flex gap-0.5 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                  <button onClick={() => openEdit(cat)} className="rounded p-1 text-slate-400 hover:text-blue-600"><Pencil className="h-3 w-3" /></button>
          ...
          <div className="animate-scale-in w-full max-w-md rounded-t-2xl sm:rounded-2xl bg-white p-5 sm:p-6 shadow-2xl mx-0 sm:mx-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                ...
                <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required className="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10" placeholder="Category name" />
              </div>
              <div>
                ...
                <div className="flex flex-wrap gap-2.5">
                  {COLORS.map((color) => (
                    <button key={color} type="button" onClick={() => setForm({ ...form, color })}
                      className={`h-8 w-8 rounded-full transition-all ${form.color === color ? 'ring-2 ring-slate-900 ring-offset-2' : 'hover:scale-110'}`}
