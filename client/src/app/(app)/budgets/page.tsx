  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button onClick={() => setShowModal(true)} className="flex items-center justify-center gap-2 rounded-xl bg-slate-900 px-5 py-2.5 text-sm font-semibold text-white hover:bg-slate-800">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:gap-4">
        <div className="flex-1 rounded-2xl bg-slate-900 p-5 text-white">
          <p className="text-xs font-medium uppercase tracking-wider text-slate-400">Total budget</p>
          <p className="mt-1 text-xl font-bold sm:text-2xl">{formatCurrency(totalBudget)}</p>
        </div>
      ...
      {budgetsList.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-slate-200 py-14 text-center">
          ...
          <button onClick={() => setShowModal(true)} className="mt-3 text-sm font-semibold text-blue-600 hover:text-blue-700">Set your first budget</button>
        </div>
      ) : (
        <div className="flex flex-col gap-4 sm:flex-row sm:flex-wrap">
          {budgetsList.map((budget) => {
            ...
            return (
              <div key={budget.id} className="w-full rounded-2xl border border-slate-100 bg-white p-5 sm:w-[calc(50%-8px)]">
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    <div className="flex h-10 w-10 items-center justify-center rounded-xl" style={{ backgroundColor: budget.category.color + '12' }}>
